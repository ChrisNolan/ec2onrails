...
* time to get the instance ready for ec2onrails (i.e. install all the default packages and gems, as well as copy over all the server/files for the default configuration).  The script will end with a command you need to run to finish creating an amazon ami-image. <pre>bash /mnt/ec2ubuntu-build-ami \ 
--codename hardy \ 
